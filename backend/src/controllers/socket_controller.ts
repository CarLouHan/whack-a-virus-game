...
export const handleConnection = (
	...
	const endGame = async (roomId: string) => {
		...
		for (const user of users) {
			if (user.scores.length > 0) {
				const averageReactionMs =
					user.scores.reduce((sum, current) => sum + current, 0) /
					user.scores.length;
		...
		await saveMatchResult(userOne.nickname, userTwo.nickname, winner, {
			player1: totalPlayerOneScore,
			player2: totalPlayerTwoScore,
		});
		...
		await updateHighscore(
			userOne.nickname,
			GetAverageReactionMs(userOne.scores)
		);
		await updateHighscore(
			userTwo.nickname,
			GetAverageReactionMs(userTwo.scores)
		);
...
async function updateHighscore(
	...
	if (
		!existingHighscore ||
		averageReactionMs < existingHighscore.averageReactionMs
	) {
